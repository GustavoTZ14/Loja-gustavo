...
export default function Page() {
  ...
  async function sub(e: React.SubmitEvent<HTMLFormElement>) {
    ...
    await authClient.signIn.email({
      ...
      callbackURL: "http://localhost:3000/usuario"
    }, {
      onError: (ctx) => {
        ...
        <div className="flex justify-center items-center w-full h-full col-span-1">
          <div className="w-130 p-10">
            ...
            <form onSubmit={sub} className="grid gap-2">
              ...
              />
              <div className="mb-10">
                <p className="text-xs">Esqueceu sua conta?</p>
              </div>
              <button type="submit" className="bg-gray-500 rounded-sm p-2 m-auto text-white font-serif cursor-pointer w-50">Entrar</button>
            </form>
          </div>
        </div>
      </div>
